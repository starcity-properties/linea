...
export default class lineaCanvas {
  ...
  compilePath(points, closed) {
    ...
    points.forEach((point, index) => {
      if (index === 0) { path.push('M', point.x, point.y); } else if (point.radius > 0 && point.curve === 'concave') { path.push('A', point.radius, point.radius, 0, 0, 1, point.x, point.y); } else if (point.radius > 0 && point.curve === 'convex') { path.push('A', point.radius, point.radius, 0, 0, 0, point.x, point.y); } else { path.push('L', point.x, point.y); }
    });
    if (closed) {
      path.push('Z');
    }
  ...
  drawPathOutline(points, closed, outlineStyle) {
    const pathString = this.compilePath(points, closed);
  ...
  drawLine(poinstObject, lineStyle) {
    const points = this.generateLineArray(poinstObject);
  ...
  drawPolygon(pointObject, style) {
    const points = this.generateLineArray(pointObject);
...
export class Room extends lineaCanvas {
  ...
  drawRoom(room, origin) {
    // console.log(room);
    const roomOutline = this.addOrigin(room.outline, origin);
...
export class Features extends lineaCanvas {
  ...
  drawFeatures(featureObjs, origin) {
    if (featureObjs === undefined) { return; }
    const interiorWalls = this.filterFeature(featureObjs, 'interiorWall');
    const doors = this.filterFeature(featureObjs, 'door');
    const slidingDoors = this.filterFeature(featureObjs, 'slidingDoor');
    const windows = this.filterFeature(featureObjs, 'window');
    const beds = this.filterFeature(featureObjs, 'bed');
    const dressers = this.filterFeature(featureObjs, 'dresser');
    const nightTables = this.filterFeature(featureObjs, 'nightTable');
  ...
  drawWindow(window, origin, windowStyle) {
    const newOutline = this.addOrigin(window.outline, origin);
  ...
  drawInteriorWall(wall, origin, wallStyle) {
    const newOutline = this.addOrigin(wall.outline, origin);
  ...
  drawSlidingDoor(slidingDoor, origin, slidingDoorStyle) {
    const slidingDoorLines = [];
    const newOutline = this.addOrigin(slidingDoor.outline, origin);
    const doorSegment = [newOutline[0], newOutline[1]];
    const dottedSegment = [newOutline[1], newOutline[2]];
  ...
  drawDoor(door, origin, doorStyle) {
    ...
    const doorStop = [];
    const doorCurve = [];
    const newOutline = this.addOrigin(door.outline, origin);
    const curve = door.clockwise ? 'concave' : 'convex';
    const hingePoint = newOutline[0];
    const endPoint = newOutline[1];
    const radius = this.lineLength(hingePoint, endPoint);

    const pointThree = {
      x: 0, y: 0, radius, curve,
    };

    // if there's angleDegrees, we set the angle, otherwise its false
    const angle = door.angleDegrees !== undefined ? door.angleDegrees : 90;
    const baseAngle = Snap.rad(Snap.angle(hingePoint.x, hingePoint.y, endPoint.x, endPoint.y)) - Math.PI;
    const supportAngle = this.getSupportAngle(baseAngle, hingePoint, endPoint, radius);
    const doorAngle = this.getDoorAngle(curve, baseAngle, angle, supportAngle);
  ...
  drawBed(bed, origin, bedStyle) {
    const bedObjs = [];
    const newOutline = this.addOrigin(bed.outline, origin);
    const topLeftPoint = newOutline[0];
    const topRightPoint = newOutline[1];

    const pillowBuffer = 2;
    const pillowLength = (this.lineLength(topLeftPoint, topRightPoint) - pillowBuffer * 3) / 2;
    const pillowHeight = pillowLength * 0.66;

    // only used for label... do we write the label here?
    const bottomRightPoint = newOutline[2];
    const centerPoint = this.lineMidPoint(topLeftPoint, bottomRightPoint);

    const pillowOnePoints = this.getPillowPoints(pillowBuffer, pillowBuffer, pillowLength, pillowHeight, topLeftPoint);
    const pillowTwoPoints = this.getPillowPoints(pillowLength + pillowBuffer * 2, pillowBuffer, pillowLength, pillowHeight, topLeftPoint);

    const pillowOne = this.drawPolygon(pillowOnePoints, bedStyle);
    const pillowTwo = this.drawPolygon(pillowTwoPoints, bedStyle);
    const drawnBed = this.drawPolygon(bed.outline, bedStyle);

    const pillowAngle = Snap.angle(topRightPoint.x, topRightPoint.y, topLeftPoint.x, topLeftPoint.y);

    this.rotateObject(pillowOne, pillowAngle, topLeftPoint);
    this.rotateObject(pillowTwo, pillowAngle, topLeftPoint);

    const label = this.paper.text(centerPoint.x + pillowHeight / 2, centerPoint.y, bed.label);
  ...
  drawRectWithLabel(rect, origin, rectStyle) {
    const rectObjs = [];
    const newOutline = this.addOrigin(rect.outline, origin);
    const centerPoint = this.lineMidPoint(newOutline[0], newOutline[2]);
    rectObjs.push(
...
class Labels extends lineaCanvas {
  ...
  drawDiagonal(a, b) {
    const d = Snap.len(a.x, a.y, b.x, b.y);
    let label = [];
    const m = this.lineMidPoint(a, b);
    const style = { textAnchor: 'middle', fontStyle: 'Arial', fontSize: 10 };
